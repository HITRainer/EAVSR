...
class L1WeightLoss(torch.nn.Module):
    def __init__(self):
        super(L1WeightLoss, self).__init__()

    def forward(self, data_coord, data_xy):
        # ex_ey = torch.exp(data_xy[:,0:1].abs()) + torch.exp(data_xy[:,1:2].abs())
        # ex_ey = ex_ey / 3.3
        ex_ey = (data_xy[:,0:1]**2 + data_xy[:,1:2]**2) * 2
        diff = (data_coord - data_xy).abs()
        diff = diff * ex_ey
        return diff.mean()
