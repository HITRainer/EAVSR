...
class BaseModel(ABC):
	...
	def forward_chop(self, lr, model, shave=32, min_size=160000):
		scale = self.scale
		n_GPUs = len(self.gpu_ids)
		n, c, h, w = lr.shape
		h_half, w_half = ((h//shave)*shave)//2, ((w//shave)*shave)//2
	...
	def get_current_visuals(self):
		...
		for name in self.visual_names:
			...
			elif 'seq' in name and self.isTrain:
				visual_ret[name] = torch.clamp(
							getattr(self, name).detach()[:, 0, ...]*255, 0, 255).round()
			else:
				visual_ret[name] = torch.clamp(
	...
	def get_backwarp(self, tenFirst, tenSecond, net, flow=None, scale=1):
		if flow is None:
			tenSecond_ = F.interpolate(tenSecond, scale_factor=1/scale, mode='bilinear', align_corners=True)
			flow = self.get_flow(tenFirst, tenSecond_, net)
			flow = F.interpolate(flow, scale_factor=scale, mode='nearest') * scale
