...
class P50EDVRDataset(BaseDataset):
	def __init__(self, opt, split='train', dataset_name='RealVSR'):
		super(P50EDVRDataset, self).__init__(opt, split, dataset_name)
		if not opt.dataroot == '':
			self.root = opt.dataroot
		else:
			if self.root == '':
				rootlist = ['/data/wrh/datasets/P50']
		...
		if split == 'train':
			self.train_list = np.load('./options/train_mvsr4x.npy')
	...
	def _getitem_train(self, idx):
		...
		folder, frame = train_name[:3], int(train_name[4:9])
	...
	def _getitem_val(self, idx):
		...
		folder, frame = val_name[:3], int(val_name[4:9])
		frame_seq = [0] * self.n_frame
		hr_seq = [0] * self.n_frame
		# front
		if frame - (self.n_frame//2) < 0:
	...
	def _get_image_dir(self, datalist, isTrain=True):
		...
		for file_name in datalist:
			folder, file = str(file_name[:3]), str(file_name[4:]) 
			if isTrain: 
				lr_dirs.append(os.path.join(self.root, 'wide', folder, file))
				hr_dirs.append(os.path.join(self.root, 'tele', folder, file))
			else:
				lr_dirs.append(os.path.join(self.root, 'wide', folder, file))
				hr_dirs.append(os.path.join(self.root, 'tele', folder, file))
